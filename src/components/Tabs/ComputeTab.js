...
If a category cannot be supported because files are missing, skip it without comment. Never invent content.
Guidelines for Writing Each "prompt":
The prompt must force the model to:
Strictly rely only on the listed files.
Explicitly verify numbers, facts, and claims with clear references.
Be deterministic and avoid hallucinations, assumptions, or extrapolations.
Ultimate Goal: Help an underwriter assess risk accurately and clearly.
Instructions to impose on the output generated by the prompt:
Output must be simple, professional, and concise.
Output must be structured as summarized bullet points wherever possible.
Focus only on critical, underwriting-relevant information.
Cut out redundant, marginal, or less useful details.
Organize outputs in a clean, structured way.
Do not overlap categories. Each must be distinct.
Do not add any commentary or filler outside the JSON array.
  `;
